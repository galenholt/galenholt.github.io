...
I'll use the parallel inner and outer functions developed in [testing nested paralellism](nested_parallel.qmd), but modified as in most of the [slurm_r\_tests repo](https://github.com/galenholt/slurm_r_tests) to just return PIDs and other diagnostics so we can see what resources get used.
...
I did this in `nested_plan.R` in the [slurm_r\_tests repo](https://github.com/galenholt/slurm_r_tests), and running it for 25 loops in each of the nested functions shows that we're getting *and using* 12 inner PIDS per outer PID
...
For more detail, run the code in the slurm_r\_tests repo. But it shows that we get 25 nodes and 12 cores each (because of `ntasks.per.node = 12` in `resources`), each of which gets used 2-3 times, which makes sense for a 25 outer x 25 inner looping.
