...
fit_nbinomAR <- function(X) {
  ...
  musize <- fitdistr::fitdist(X, 'nbinom')
  # check the AR
  ac_x1 <- acf(X)$acf[2]
...
Standard checks using the frequencies and then a chi square or a plot. 
These just check the distribution, not the AR. We'll have to take the rho estimation from acf's word on that.
