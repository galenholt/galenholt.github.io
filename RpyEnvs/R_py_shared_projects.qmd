...
I've done a bit getting them to [work in the same *script*](RandPython.qmd)*,* and setting up [clean python environments with poetry](python_setyup.qmd). With the shared scripts, I did it with a subdirectory, and that sort of worked for testing, but won't work for a project where they're both used a fair amount and in both places.
