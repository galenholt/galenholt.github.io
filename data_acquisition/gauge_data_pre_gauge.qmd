...
We often end up needing to pull gauge data for lots of projects. If we just want flow data, and especially if we're working in python, there's the [mdba_gauge_getter](https://github.com/MDBAuth/MDBA_Gauge_Getter), and I've written [{vicwater}](https://github.com/galenholt/vicwater) for R and expanded it to cover Victoria, NSW, and Qld. A primary advantage of {vicwater} is that it can access more of the API calls and return any desired variables.
...
The cleaner way is to use `get_ts_traces2`, which does the hunting for dates automatically when given `start_time = 'all'` and `end_time = 'all'`.

```{r}
siteout_a <- get_ts_traces2(state = 'NSW', 
                            site_list = c('422028', '410007'), 
                            var_list = "141",
                            start_time = 'all',
                            end_time = 'all',
